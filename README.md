...
+ [`Supplementary`](Supplementary) contains the code for supplementary figures for [`Supplementary`](Supplementary)

Main analysis steps are included as follows:
+ [`3_limma_EMCI_META.ipynb`](3_limma_EMCI_META.ipynb) contains the differential analysis based on the metabolomics on the EMCI subtypes.
+ [`3_limma_EMCI_MRNA.ipynb`](3_limma_EMCI_MRNA.ipynb) contains the differential analysis based on the MRNA on the EMCI subtypes.
+ [`3_limma_LMCI_META.ipynb`](3_limma_LMCI_META.ipynb) contains the differential analysis based on the metabolomics on the LMCI subtypes.
+ [`3_limma_EMCI_MRNA.ipynb`](3_limma_LMCI_MRNA.ipynb) contains the differential analysis based on the MRNA on the LMCI subtypes.
...
+ [`4_GSEA.ipynb`](4_GSEA.ipynb)contains the gene set enrichment analysis based on differential analysis results
+ [`5_clintrajan.ipynb`](5_clintrajan.ipynb)contains the trajecotry analysis of different subgroups based on their clinical variables.
+ [`5_prepare_time_to_diagnosis.ipynb`](5_prepare_time_to_diagnosis.ipynb) is for preparing the time to diagnoisis analysis plot
+ [`5_time_to_diagnosis.ipynb`](5_time_to_diagnosis.ipynb)contains the Time-to-diagnosis analysis for the percentage of dementia in every six-month check up, among the EMCI and LMCI subtypes.
+ [`6_Preprocess_EFIGA.R`](6_Preprocess_EFIGA.R) contains EFIGA data preprocessing for later label transfering.
+ [`6_Unioncom_label_transfer.py`](6_Unioncom_label_transfer.py) contains label transfering using ADNI labels and EFIGA metabolites data using Unioncom for MCI subtypes.
+ [`6_Unioncom_refined_label_transfer.py`](6_Unioncom_refined__label_transfer.py) contains label transfering using ADNI labels and EFIGA metabolites data using Unioncom for refined MCI subtypes.
+ Other code are for result visualization.
